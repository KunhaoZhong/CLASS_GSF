...
int background_functions(
  ...
  double phi, phi_prime, X, P, PX, Pphi, PXX, PXphi, Pphiphi, PXXX, PXXphi, PXphiphi;
  ...
  /* Scalar field */
if (pba->has_gsf == _TRUE_) 
  {
    // H has not been calculated here, should not matter. Those depend on H will be calculated later.
    phi = pvecback_B[pba->index_bi_phi_gsf];
    phi_prime = pvecback_B[pba->index_bi_phi_prime_gsf];
    pvecback[pba->index_bg_phi_gsf] = phi; // value of the scalar field phi
    pvecback[pba->index_bg_phi_prime_gsf] = phi_prime; 
    //double *gsf_in = malloc(sizeof(double[4])); 
    double gsf_in[4];
    double gsf_out[12];
    gsf_in[0] = a;
    gsf_in[1] = 0; //H havn't been calculated yet, postpone to the end of this function
    gsf_in[2] = phi;
    gsf_in[3] = phi_prime;


    //double *gsf_out = malloc(sizeof(double[12]));
    gsf_functions(pba, gsf_in, gsf_out );
    //free(gsf_in);
    pvecback[pba->index_bg_P_gsf] = gsf_out[0];
    pvecback[pba->index_bg_PX_gsf] = gsf_out[1];
    pvecback[pba->index_bg_Pphi_gsf] = gsf_out[2];
    pvecback[pba->index_bg_PXX_gsf] = gsf_out[3];
    pvecback[pba->index_bg_PXphi_gsf] = gsf_out[4];
    pvecback[pba->index_bg_Pphiphi_gsf] = gsf_out[5];
    pvecback[pba->index_bg_PXXX_gsf] = gsf_out[6];
    pvecback[pba->index_bg_PXXphi_gsf] = gsf_out[7];
    pvecback[pba->index_bg_PXphiphi_gsf] = gsf_out[8];

    pvecback[pba->index_bg_rho_gsf] = gsf_out[9];
    pvecback[pba->index_bg_p_gsf] = gsf_out[10];
    pvecback[pba->index_bg_phi_prime_prime_gsf] = gsf_out[11];
      //free(gsf_out);

    rho_tot += pvecback[pba->index_bg_rho_gsf];
    p_tot += pvecback[pba->index_bg_p_gsf];
    dp_dloga += 0.0; /** <-- This depends on a_prime_over_a, so we cannot add it now! */
  }
  // The second non-cononical scalar field, serve as EDE, so no shooting for this component
  if (pba->has_gsf2 == _TRUE_) {
    // H has not been calculated here, should not matter. Those depend on H will be calculated later.
    phi = pvecback_B[pba->index_bi_phi_gsf2];
    phi_prime = pvecback_B[pba->index_bi_phi_prime_gsf2];
    pvecback[pba->index_bg_phi_gsf2] = phi; // value of the scalar field phi
    pvecback[pba->index_bg_phi_prime_gsf2] = phi_prime; 
    //double *gsf2_in = malloc(sizeof(double[4])); 
    double gsf2_in[4];
    double gsf2_out[12];
    gsf2_in[0] = a;
    gsf2_in[1] = 0; //H havn't been calculated yet, postpone to the end of this function
    gsf2_in[2] = phi;
    gsf2_in[3] = phi_prime;


    //double *gsf2_out = malloc(sizeof(double[12]));
    gsf2_functions(pba, gsf2_in, gsf2_out);
    //free(gsf2_in);
    pvecback[pba->index_bg_P_gsf2] = gsf2_out[0];
    pvecback[pba->index_bg_PX_gsf2] = gsf2_out[1];
    pvecback[pba->index_bg_Pphi_gsf2] = gsf2_out[2];
    pvecback[pba->index_bg_PXX_gsf2] = gsf2_out[3];
    pvecback[pba->index_bg_PXphi_gsf2] = gsf2_out[4];
    pvecback[pba->index_bg_Pphiphi_gsf2] = gsf2_out[5];
    pvecback[pba->index_bg_PXXX_gsf2] = gsf2_out[6];
    pvecback[pba->index_bg_PXXphi_gsf2] = gsf2_out[7];
    pvecback[pba->index_bg_PXphiphi_gsf2] = gsf2_out[8];

    pvecback[pba->index_bg_rho_gsf2] = gsf2_out[9];
    pvecback[pba->index_bg_p_gsf2] = gsf2_out[10];
    pvecback[pba->index_bg_phi_prime_prime_gsf2] = gsf2_out[11];
    //free(gsf2_out);

    rho_tot += pvecback[pba->index_bg_rho_gsf2];
    p_tot += pvecback[pba->index_bg_p_gsf2];
    dp_dloga += 0.0; /** <-- This depends on a_prime_over_a, so we cannot add it now! */
  }
 ...
 /* Add a Hubble friction term as in Hi-class, with friction =3  */
 pvecback[pba->index_bg_H_prime] = - (3./2.) * (rho_tot + p_tot) * a + pba->K/a - a* 3*(pvecback[pba->index_bg_H]*pvecback[pba->index_bg_H] - (rho_tot-pba->K/a/a) );
  ...
  gsf_in[0] = a;
  gsf_in[1] = pvecback[pba->index_bg_H]; 
  gsf_in[2] = pvecback[pba->index_bg_phi_gsf];
  gsf_in[3] = pvecback[pba->index_bg_phi_prime_gsf];
  ...
  double curlyH = pvecback[pba->index_bg_H]*a;
  double phi = pvecback[pba->index_bg_phi_gsf];
  double phi_prime = pvecback[pba->index_bg_phi_prime_gsf];
  pvecback[pba->index_bg_P_gsf] = gsf_out[0];
  pvecback[pba->index_bg_PX_gsf] = gsf_out[1];
  pvecback[pba->index_bg_Pphi_gsf] = gsf_out[2];
  pvecback[pba->index_bg_PXX_gsf] = gsf_out[3];
  ...
  double P  = pvecback[pba->index_bg_P_gsf];
  double PX = pvecback[pba->index_bg_PX_gsf];
  double Pphi = pvecback[pba->index_bg_Pphi_gsf];
  double PXX = pvecback[pba->index_bg_PXX_gsf];
  double PXphi = pvecback[pba->index_bg_PXphi_gsf];
  double Pphiphi = pvecback[pba->index_bg_Pphiphi_gsf];
  double PXXX = pvecback[pba->index_bg_PXXX_gsf];
  double PXXphi  = pvecback[pba->index_bg_PXXphi_gsf];
  double PXphiphi = pvecback[pba->index_bg_PXphiphi_gsf];
  //p' of gsf
  pvecback[pba->index_bg_p_prime_gsf] = PX*(gsf_in[3]*gsf_out[11]/a/a - gsf_in[2]*gsf_in[2]*PX/a) + Pphi*gsf_in[3];
  pvecback[pba->index_bg_p_tot_prime] += pvecback[pba->index_bg_p_prime_gsf];

  // cs2 of gsf, defined as the speed of sound in 9904176
  double X = (gsf_in[3]*gsf_in[3])/(2*a*a);
  double X_prime = (phi_prime*gsf_out[11]-curlyH*phi_prime*phi_prime)/(a*a);
  double phi_prime_square = phi_prime*phi_prime;
  double phi_prime_prime = gsf_out[11];
  double a2 = a*a;
  pvecback[pba->index_bg_cs2_gsf] = PX/(2*X*PXX+ PX);
  pvecback[pba->index_bg_w_gsf] = pvecback[pba->index_bg_p_gsf] /pvecback[pba->index_bg_rho_gsf] ;

  pvecback[pba->index_bg_A_gsf] = PXX*phi_prime*phi_prime/a2 + PX; 
  pvecback[pba->index_bg_B_gsf] = 2*curlyH*PX + PXXX * X_prime*phi_prime_square/2 + PXXphi*phi_prime_square*phi_prime/a2 + PXX*( 3*phi_prime*phi_prime_prime/a2 - curlyH*phi_prime_square/a2 ) + PXphi*phi_prime ;
  pvecback[pba->index_bg_C_dif_gsf] = PXXphi*X_prime*phi_prime + PXphi*phi_prime_prime + PXphiphi*phi_prime_square + 2*curlyH*PXphi*phi_prime;
  ...
  double gsf2_out[12];
  gsf2_in[0] = a;
  gsf2_in[1] = pvecback[pba->index_bg_H]; 
  gsf2_in[2] = pvecback[pba->index_bg_phi_gsf2];
  gsf2_in[3] = pvecback[pba->index_bg_phi_prime_gsf2];
  ...
  double curlyH = pvecback[pba->index_bg_H]*a;
  double phi = pvecback[pba->index_bg_phi_gsf2];
  double phi_prime = pvecback[pba->index_bg_phi_prime_gsf2];
  pvecback[pba->index_bg_P_gsf2] = gsf2_out[0];
  pvecback[pba->index_bg_PX_gsf2] = gsf2_out[1];
  pvecback[pba->index_bg_Pphi_gsf2] = gsf2_out[2];
  pvecback[pba->index_bg_PXX_gsf2] = gsf2_out[3];
  ...
  double P  = pvecback[pba->index_bg_P_gsf2];
  double PX = pvecback[pba->index_bg_PX_gsf2];
  double Pphi = pvecback[pba->index_bg_Pphi_gsf2];
  double PXX = pvecback[pba->index_bg_PXX_gsf2];
  double PXphi = pvecback[pba->index_bg_PXphi_gsf2];
  double Pphiphi = pvecback[pba->index_bg_Pphiphi_gsf2];
  double PXXX = pvecback[pba->index_bg_PXXX_gsf2];
  double PXXphi  = pvecback[pba->index_bg_PXXphi_gsf2];
  double PXphiphi = pvecback[pba->index_bg_PXphiphi_gsf2];

  //p' of gsf
  pvecback[pba->index_bg_p_prime_gsf2] = PX*(gsf2_in[3]*gsf2_out[11]/a/a - gsf2_in[2]*gsf2_in[2]*PX/a) + Pphi*gsf2_in[3];
  pvecback[pba->index_bg_p_tot_prime] += pvecback[pba->index_bg_p_prime_gsf2];

  // cs2 of gsf, defined as the speed of sound in 9904176
  double X = (gsf2_in[3]*gsf2_in[3])/(2*a*a);
  double X_prime = (phi_prime*gsf2_out[11]-curlyH*phi_prime*phi_prime)/(a*a);
  double phi_prime_square = phi_prime*phi_prime;
  double phi_prime_prime = gsf2_out[11];
  double a2 = a*a;
  pvecback[pba->index_bg_cs2_gsf2] = PX/(2*X*PXX+ PX);
  pvecback[pba->index_bg_w_gsf2] = pvecback[pba->index_bg_p_gsf2] /pvecback[pba->index_bg_rho_gsf2] ;

  pvecback[pba->index_bg_A_gsf2] = PXX*phi_prime*phi_prime/a2 + PX; 
  pvecback[pba->index_bg_B_gsf2] = 2*curlyH*PX + PXXX * X_prime*phi_prime_square/2 + PXXphi*phi_prime_square*phi_prime/a2 + PXX*( 3*phi_prime*phi_prime_prime/a2 - curlyH*phi_prime_square/a2 ) + PXphi*phi_prime ;
  pvecback[pba->index_bg_C_dif_gsf2] = PXXphi*X_prime*phi_prime + PXphi*phi_prime_prime + PXphiphi*phi_prime_square + 2*curlyH*PXphi*phi_prime;
...
int background_free_input(
  ...
  if (pba->Omega0_gsf != 0.) {
    if (pba->gsf_parameters != NULL)
      free(pba->gsf_parameters);
  }
    if (pba->Omega0_gsf2 != 0.) {
    if (pba->gsf2_parameters != NULL)
      free(pba->gsf2_parameters);
  }
  return _SUCCESS_;
}
...
int background_indices(
  ...
  pba->has_dr = _FALSE_;
  pba->has_gsf = _FALSE_;
  // pba->has_gsf2 = _FALSE_; //from input file: want_gsf2
  ...
  class_define_index(pba->index_bg_phi_gsf,pba->has_gsf,index_bg,1);
  class_define_index(pba->index_bg_phi_prime_gsf,pba->has_gsf,index_bg,1);
  class_define_index(pba->index_bg_phi_prime_prime_gsf,pba->has_gsf,index_bg,1);
  class_define_index(pba->index_bg_V_gsf,pba->has_gsf,index_bg,1);
  class_define_index(pba->index_bg_dV_gsf,pba->has_gsf,index_bg,1);
  class_define_index(pba->index_bg_ddV_gsf,pba->has_gsf,index_bg,1);
  class_define_index(pba->index_bg_rho_gsf,pba->has_gsf,index_bg,1);
  class_define_index(pba->index_bg_p_gsf,pba->has_gsf,index_bg,1);
  class_define_index(pba->index_bg_p_prime_gsf,pba->has_gsf,index_bg,1);
 class_define_index(pba->index_bg_cs2_gsf,pba->has_gsf,index_bg,1);
  class_define_index(pba->index_bg_w_gsf,pba->has_gsf,index_bg,1);


  class_define_index(pba->index_bg_P_gsf,pba->has_gsf,index_bg,1);
  class_define_index(pba->index_bg_PX_gsf,pba->has_gsf,index_bg,1);
  class_define_index(pba->index_bg_Pphi_gsf,pba->has_gsf,index_bg,1);
  class_define_index(pba->index_bg_PXX_gsf,pba->has_gsf,index_bg,1);
  class_define_index(pba->index_bg_PXphi_gsf,pba->has_gsf,index_bg,1);
  class_define_index(pba->index_bg_Pphiphi_gsf,pba->has_gsf,index_bg,1);
  class_define_index(pba->index_bg_PXXX_gsf,pba->has_gsf,index_bg,1);
  class_define_index(pba->index_bg_PXXphi_gsf,pba->has_gsf,index_bg,1);
  class_define_index(pba->index_bg_PXphiphi_gsf,pba->has_gsf,index_bg,1);
  class_define_index(pba->index_bg_A_gsf,pba->has_gsf,index_bg,1);
  class_define_index(pba->index_bg_B_gsf,pba->has_gsf,index_bg,1);
  class_define_index(pba->index_bg_C_dif_gsf,pba->has_gsf,index_bg,1);
  class_define_index(pba->index_bg_D_gsf,pba->has_gsf,index_bg,1);



//gsf2
  class_define_index(pba->index_bg_phi_gsf2,pba->has_gsf2,index_bg,1);
  class_define_index(pba->index_bg_phi_prime_gsf2,pba->has_gsf2,index_bg,1);
  class_define_index(pba->index_bg_phi_prime_prime_gsf2,pba->has_gsf2,index_bg,1);
  class_define_index(pba->index_bg_V_gsf2,pba->has_gsf2,index_bg,1);
  class_define_index(pba->index_bg_dV_gsf2,pba->has_gsf2,index_bg,1);
  class_define_index(pba->index_bg_ddV_gsf2,pba->has_gsf2,index_bg,1);
  class_define_index(pba->index_bg_rho_gsf2,pba->has_gsf2,index_bg,1);
  class_define_index(pba->index_bg_p_gsf2,pba->has_gsf2,index_bg,1);
  class_define_index(pba->index_bg_p_prime_gsf2,pba->has_gsf2,index_bg,1);
  class_define_index(pba->index_bg_cs2_gsf2,pba->has_gsf2,index_bg,1);
  class_define_index(pba->index_bg_w_gsf2,pba->has_gsf2,index_bg,1);

  class_define_index(pba->index_bg_P_gsf2,pba->has_gsf2,index_bg,1);
  class_define_index(pba->index_bg_PX_gsf2,pba->has_gsf2,index_bg,1);
  class_define_index(pba->index_bg_Pphi_gsf2,pba->has_gsf2,index_bg,1);
  class_define_index(pba->index_bg_PXX_gsf2,pba->has_gsf2,index_bg,1);
  class_define_index(pba->index_bg_PXphi_gsf2,pba->has_gsf2,index_bg,1);
  class_define_index(pba->index_bg_Pphiphi_gsf2,pba->has_gsf2,index_bg,1);
  class_define_index(pba->index_bg_PXXX_gsf2,pba->has_gsf2,index_bg,1);
  class_define_index(pba->index_bg_PXXphi_gsf2,pba->has_gsf2,index_bg,1);
  class_define_index(pba->index_bg_PXphiphi_gsf2,pba->has_gsf2,index_bg,1);
  class_define_index(pba->index_bg_A_gsf2,pba->has_gsf2,index_bg,1);
  class_define_index(pba->index_bg_B_gsf2,pba->has_gsf2,index_bg,1);
  class_define_index(pba->index_bg_C_dif_gsf2,pba->has_gsf2,index_bg,1);
  class_define_index(pba->index_bg_D_gsf2,pba->has_gsf2,index_bg,1);
  /* - index for Lambda */
  ...
  class_define_index(pba->index_bi_phi_gsf,pba->has_gsf,index_bi,1);
  class_define_index(pba->index_bi_phi_prime_gsf,pba->has_gsf,index_bi,1);

  class_define_index(pba->index_bi_phi_gsf2,pba->has_gsf2,index_bi,1);
  class_define_index(pba->index_bi_phi_prime_gsf2,pba->has_gsf2,index_bi,1);
...
int background_solve(
  ...
  int index_loga, index_gsf, index_gsf2;
  ...
  // Maybe in a seperate function background_find_ede_parameters
  /** gsf-edit: for early dark energy 
   * Find f_ede and z_c
   * 
   */

   // printf("start to find f_ede and z_c");
    if (pba->has_gsf2 == _TRUE_){
      int i;
      double max = 0;
      for (i=0; i < pba->bt_size; i++) {

  //  printf("test= %f", pvecback[pba->index_bg_rho_crit]);
     class_call(background_at_z(pba,
		 ...
		 pba->error_message,
		 pba->error_message);
    //   // class_call(background_at_tau(pba,pba->tau_table[i],long_info,inter_closeby,0,pvecback),
    //   //          pba->error_message,
    //   //          pba->error_message);


       double Omega_ede = pvecback[pba->index_bg_rho_gsf2]/ pvecback[pba->index_bg_rho_crit];
      //  printf("test= %f", Omega_ede);
      if (Omega_ede > max){
          pba->f_ede = Omega_ede;
          pba->zc_ede = pba->z_table[i];
          max = Omega_ede;
      }
    
       }
 //    printf("f_ede = %f, zc_ede = %f \n", pba->f_ede, pba->zc_ede);
    }
  ...
  if (pba->background_verbose > 2) {
    ...
    if (pba->has_gsf == _TRUE_) {
      printf("    Kessence Scalar field details:\n");
      printf("     -> Omega_gsf = %g, wished %g\n",
             pba->background_table[(pba->bt_size-1)*pba->bg_size+pba->index_bg_rho_gsf]/pba->background_table[(pba->bt_size-1)*pba->bg_size+pba->index_bg_rho_crit], pba->Omega0_gsf);
      if (pba->has_lambda == _TRUE_) {
      ...
      printf("     -> parameters: [check background.c] = \n");
      printf("                    [");
      for (index_gsf=0; index_gsf<pba->gsf_parameters_size-1; index_gsf++) {
        printf("%.3f, ",pba->gsf_parameters[index_gsf]);
      }
      printf("%.3f]\n",pba->gsf_parameters[pba->gsf_parameters_size-1]);
    }
    if (pba->has_gsf2 == _TRUE_) {
      printf("    Second Scalar field details:\n");
      printf("     -> parameters: [check background.c] = \n");
      printf("                    [");
      for (index_gsf2=0; index_gsf2<pba->gsf2_parameters_size-1; index_gsf2++) {
        printf("%.3f, ",pba->gsf2_parameters[index_gsf2]);
      }
      printf("%.3f]\n",pba->gsf2_parameters[pba->gsf2_parameters_size-1]);
    }
  }
...
int background_initial_conditions(
  ...
  double gsf_lambda, c1, x1, x2, gsf2_lambda;
  ...
  */
  ...
  if (pba->has_gsf == _TRUE_) {
    ...
    x1 = pba->gsf_parameters[1]*pow(10, pba->gsf_parameters[2]);
    x2 = pba->gsf_parameters[3]*pow(10, pba->gsf_parameters[4]);

//should not use attractor ic at this time.
    if (pba->attractor_ic_gsf == _TRUE_) {
      // pvecback_integration[pba->index_bi_phi_gsf] = log(1/(4*x1*x1*rho_rad*c1))/gsf_lambda;
      // pvecback_integration[pba->index_bi_phi_prime_gsf] = (0.5+x2)*sqrt(2)*a;
      printf("Attractor IC not implemented, because it's model dependent");
    }
    else {
      printf("Not using attractor initial conditions\n");
      /** - --> If no attractor initial conditions are assigned, gets the provided ones. */
      pvecback_integration[pba->index_bi_phi_gsf] = x1;
      pvecback_integration[pba->index_bi_phi_prime_gsf] = x2;
    }
    class_test(!isfinite(pvecback_integration[pba->index_bi_phi_gsf]) ||
               !isfinite(pvecback_integration[pba->index_bi_phi_gsf]),
               pba->error_message,
               "initial phi = %e phi_prime = %e -> check initial conditions",
               pvecback_integration[pba->index_bi_phi_gsf],
               pvecback_integration[pba->index_bi_phi_gsf]);
  }
 ////gsf2
    if (pba->has_gsf2 == _TRUE_) {
//should not use attractor ic at this time.
    if (pba->attractor_ic_gsf2 == _TRUE_) {
      // pvecback_integration[pba->index_bi_phi_gsf] = log(1/(4*x1*x1*rho_rad*c1))/gsf_lambda;
      // pvecback_integration[pba->index_bi_phi_prime_gsf] = (0.5+x2)*sqrt(2)*a;
      printf("Attractor IC not implemented, because it's model dependent");
    }
      ...
      //printf("Not using attractor initial conditions\n");
      /** - --> If no attractor initial conditions are assigned, gets the provided ones. */
      //pvecback_integration[pba->index_bi_phi_gsf2] = pba->gsf2_parameters[1]*pow(10, pba->gsf2_parameters[2]);
      pvecback_integration[pba->index_bi_phi_gsf2] = pba->ede_p1;
      pvecback_integration[pba->index_bi_phi_prime_gsf2] = 0;
      //printf("ede_p1 = %f", pba->ede_p1);
      //pvecback_integration[pba->index_bi_phi_prime_gsf2] = pba->gsf2_parameters[3]*pow(10, pba->gsf2_parameters[4]);
    }
    class_test(!isfinite(pvecback_integration[pba->index_bi_phi_gsf2]) ||
               !isfinite(pvecback_integration[pba->index_bi_phi_gsf2]),
               pba->error_message,
               "initial phi = %e phi_prime = %e -> check initial conditions",
               pvecback_integration[pba->index_bi_phi_gsf2],
               pvecback_integration[pba->index_bi_phi_gsf2]);
  }
...
int background_output_titles(
  ...
  class_store_columntitle(titles,"(.)rho_gsf",pba->has_gsf);
  class_store_columntitle(titles,"(.)p_gsf",pba->has_gsf);
  class_store_columntitle(titles,"(.)p_prime_gsf",pba->has_gsf);
  class_store_columntitle(titles,"(.)cs2_gsf",pba->has_gsf);
  class_store_columntitle(titles,"(.)w_gsf",pba->has_gsf);
  class_store_columntitle(titles,"phi_gsf",pba->has_gsf);
  class_store_columntitle(titles,"phi'_gsf",pba->has_gsf);
  class_store_columntitle(titles,"phi''_gsf",pba->has_gsf);
  class_store_columntitle(titles,"V_gsf",pba->has_gsf);
  class_store_columntitle(titles,"V'_gsf",pba->has_gsf);
  class_store_columntitle(titles,"V''_gsf",pba->has_gsf);

  class_store_columntitle(titles,"P_gsf",pba->has_gsf);
  class_store_columntitle(titles,"PX_gsf",pba->has_gsf);
  class_store_columntitle(titles,"Pphi_gsf",pba->has_gsf);
  class_store_columntitle(titles,"PXX_gsf",pba->has_gsf);
  class_store_columntitle(titles,"PXphi_gsf",pba->has_gsf);
  class_store_columntitle(titles,"Pphiphi_gsf",pba->has_gsf);
  class_store_columntitle(titles,"PXXX_gsf",pba->has_gsf);
  class_store_columntitle(titles,"PXXphi_gsf",pba->has_gsf);
  class_store_columntitle(titles,"PXphiphi_gsf",pba->has_gsf);
  class_store_columntitle(titles,"A_gsf",pba->has_gsf);
  class_store_columntitle(titles,"B_gsf",pba->has_gsf);
  class_store_columntitle(titles,"C_dif_gsf",pba->has_gsf);
  class_store_columntitle(titles,"D_gsf",pba->has_gsf);
//gsf2
  class_store_columntitle(titles,"(.)rho_gsf2",pba->has_gsf2);
  class_store_columntitle(titles,"(.)p_gsf2",pba->has_gsf2);
  class_store_columntitle(titles,"(.)p_prime_gsf2",pba->has_gsf2);
  class_store_columntitle(titles,"(.)cs2_gsf2",pba->has_gsf2);
  class_store_columntitle(titles,"(.)w_gsf2",pba->has_gsf2);
  class_store_columntitle(titles,"phi_gsf2",pba->has_gsf2);
  class_store_columntitle(titles,"phi'_gsf2",pba->has_gsf2);
  class_store_columntitle(titles,"phi''_gsf2",pba->has_gsf2);
  class_store_columntitle(titles,"V_gsf2",pba->has_gsf2);
  class_store_columntitle(titles,"V'_gsf2",pba->has_gsf2);
  class_store_columntitle(titles,"V''_gsf2",pba->has_gsf2);

  class_store_columntitle(titles,"P_gsf2",pba->has_gsf2);
  class_store_columntitle(titles,"PX_gsf2",pba->has_gsf2);
  class_store_columntitle(titles,"Pphi_gsf2",pba->has_gsf2);
  class_store_columntitle(titles,"PXX_gsf2",pba->has_gsf2);
  class_store_columntitle(titles,"PXphi_gsf2",pba->has_gsf2);
  class_store_columntitle(titles,"Pphiphi_gsf2",pba->has_gsf2);
  class_store_columntitle(titles,"PXXX_gsf2",pba->has_gsf2);
  class_store_columntitle(titles,"PXXphi_gsf2",pba->has_gsf2);
  class_store_columntitle(titles,"PXphiphi_gsf2",pba->has_gsf2);
  class_store_columntitle(titles,"A_gsf2",pba->has_gsf2);
  class_store_columntitle(titles,"B_gsf2",pba->has_gsf2);
  class_store_columntitle(titles,"C_dif_gsf2",pba->has_gsf2);
  class_store_columntitle(titles,"D_gsf2",pba->has_gsf2);
...
int background_output_data(
  ...
  for (index_tau=0; index_tau<pba->bt_size; index_tau++) {
    ...
    class_store_double(dataptr,pvecback[pba->index_bg_rho_gsf],pba->has_gsf,storeidx);
    class_store_double(dataptr,pvecback[pba->index_bg_p_gsf],pba->has_gsf,storeidx);
    class_store_double(dataptr,pvecback[pba->index_bg_p_prime_gsf],pba->has_gsf,storeidx);
    class_store_double(dataptr,pvecback[pba->index_bg_cs2_gsf],pba->has_gsf,storeidx);
    class_store_double(dataptr,pvecback[pba->index_bg_w_gsf],pba->has_gsf,storeidx);
    class_store_double(dataptr,pvecback[pba->index_bg_phi_gsf],pba->has_gsf,storeidx);
    class_store_double(dataptr,pvecback[pba->index_bg_phi_prime_gsf],pba->has_gsf,storeidx);
    class_store_double(dataptr,pvecback[pba->index_bg_phi_prime_prime_gsf],pba->has_gsf,storeidx);
    class_store_double(dataptr,pvecback[pba->index_bg_V_gsf],pba->has_gsf,storeidx);
    class_store_double(dataptr,pvecback[pba->index_bg_dV_gsf],pba->has_gsf,storeidx);
    class_store_double(dataptr,pvecback[pba->index_bg_ddV_gsf],pba->has_gsf,storeidx);

     class_store_double(dataptr,pvecback[pba->index_bg_P_gsf],pba->has_gsf,storeidx);
    class_store_double(dataptr,pvecback[pba->index_bg_PX_gsf],pba->has_gsf,storeidx);
    class_store_double(dataptr,pvecback[pba->index_bg_Pphi_gsf],pba->has_gsf,storeidx);
    class_store_double(dataptr,pvecback[pba->index_bg_PXX_gsf],pba->has_gsf,storeidx);
    class_store_double(dataptr,pvecback[pba->index_bg_PXphi_gsf],pba->has_gsf,storeidx);
    class_store_double(dataptr,pvecback[pba->index_bg_Pphiphi_gsf],pba->has_gsf,storeidx);
    class_store_double(dataptr,pvecback[pba->index_bg_PXXX_gsf],pba->has_gsf,storeidx);
    class_store_double(dataptr,pvecback[pba->index_bg_PXXphi_gsf],pba->has_gsf,storeidx);
    class_store_double(dataptr,pvecback[pba->index_bg_PXphiphi_gsf],pba->has_gsf,storeidx);
    class_store_double(dataptr,pvecback[pba->index_bg_A_gsf],pba->has_gsf,storeidx);
    class_store_double(dataptr,pvecback[pba->index_bg_B_gsf],pba->has_gsf,storeidx);
    class_store_double(dataptr,pvecback[pba->index_bg_C_dif_gsf],pba->has_gsf,storeidx);
    class_store_double(dataptr,pvecback[pba->index_bg_D_gsf],pba->has_gsf,storeidx);


    //gsf2
    class_store_double(dataptr,pvecback[pba->index_bg_rho_gsf2],pba->has_gsf2,storeidx);
    class_store_double(dataptr,pvecback[pba->index_bg_p_gsf2],pba->has_gsf2,storeidx);
    class_store_double(dataptr,pvecback[pba->index_bg_p_prime_gsf2],pba->has_gsf2,storeidx);
    class_store_double(dataptr,pvecback[pba->index_bg_cs2_gsf2],pba->has_gsf2,storeidx);
    class_store_double(dataptr,pvecback[pba->index_bg_w_gsf2],pba->has_gsf2,storeidx);
    class_store_double(dataptr,pvecback[pba->index_bg_phi_gsf2],pba->has_gsf2,storeidx);
    class_store_double(dataptr,pvecback[pba->index_bg_phi_prime_gsf2],pba->has_gsf2,storeidx);
    class_store_double(dataptr,pvecback[pba->index_bg_phi_prime_prime_gsf2],pba->has_gsf2,storeidx);
    class_store_double(dataptr,pvecback[pba->index_bg_V_gsf2],pba->has_gsf2,storeidx);
    class_store_double(dataptr,pvecback[pba->index_bg_dV_gsf2],pba->has_gsf2,storeidx);
    class_store_double(dataptr,pvecback[pba->index_bg_ddV_gsf2],pba->has_gsf2,storeidx);

    class_store_double(dataptr,pvecback[pba->index_bg_P_gsf2],pba->has_gsf2,storeidx);
    class_store_double(dataptr,pvecback[pba->index_bg_PX_gsf2],pba->has_gsf2,storeidx);
    class_store_double(dataptr,pvecback[pba->index_bg_Pphi_gsf2],pba->has_gsf2,storeidx);
    class_store_double(dataptr,pvecback[pba->index_bg_PXX_gsf2],pba->has_gsf2,storeidx);
    class_store_double(dataptr,pvecback[pba->index_bg_PXphi_gsf2],pba->has_gsf2,storeidx);
    class_store_double(dataptr,pvecback[pba->index_bg_Pphiphi_gsf2],pba->has_gsf2,storeidx);
    class_store_double(dataptr,pvecback[pba->index_bg_PXXX_gsf2],pba->has_gsf2,storeidx);
    class_store_double(dataptr,pvecback[pba->index_bg_PXXphi_gsf2],pba->has_gsf2,storeidx);
    class_store_double(dataptr,pvecback[pba->index_bg_PXphiphi_gsf2],pba->has_gsf2,storeidx);
    class_store_double(dataptr,pvecback[pba->index_bg_A_gsf2],pba->has_gsf2,storeidx);
    class_store_double(dataptr,pvecback[pba->index_bg_B_gsf2],pba->has_gsf2,storeidx);
    class_store_double(dataptr,pvecback[pba->index_bg_C_dif_gsf2],pba->has_gsf2,storeidx);
    class_store_double(dataptr,pvecback[pba->index_bg_D_gsf2],pba->has_gsf2,storeidx);
...
int background_derivs(
  ...
  double X, Vprime, phidotdot, V, P_X, P_XX, P_Xphi,P_phi, P_phiphi;
  ...
  if (pba->has_gsf == _TRUE_) {
    /** - Kinetic Scalar field equation: \f$ \phidotdot*(-1+6X)*V+ 3Sqrt(2X)*(-1+2X)*V*H +(-X+3X^2)*V'= 0 \f$  (phi''=phidotdot*a^2+phidot*H*a^2),
     * just need to change p_x etc accordingly.
     * thus phi''= () / ()
        written as \f$ d\phi/dlna = phi' / (aH) \f$ and \f$  \f$ */
  //double *gsf_in =  malloc(sizeof(double[4]));; 
  double gsf_in[4];
  double gsf_out[12];
  gsf_in[0] = a;
  gsf_in[1] = H;
  gsf_in[2] = y[pba->index_bi_phi_gsf];
  gsf_in[3] = y[pba->index_bi_phi_prime_gsf];
  ...
  //free(gsf_out);
    ...
    dy[pba->index_bi_phi_gsf] = y[pba->index_bi_phi_prime_gsf]/a/H;
    dy[pba->index_bi_phi_prime_gsf] = phi_prime_prime/a/H;
  ...
  if (pba->has_gsf2 == _TRUE_) {
    ...
    gsf2_in[0] = a;
    gsf2_in[1] = H;
    gsf2_in[2] = y[pba->index_bi_phi_gsf2];
    gsf2_in[3] = y[pba->index_bi_phi_prime_gsf2];


    //double *gsf2_out =  malloc(sizeof(double[12]));
    gsf2_functions(pba, gsf2_in, gsf2_out);
    //free(gsf2_in);
    double phi_prime_prime_2 = gsf2_out[11];
    //free(gsf2_out);
    dy[pba->index_bi_phi_gsf2] = y[pba->index_bi_phi_prime_gsf2]/a/H;
    dy[pba->index_bi_phi_prime_gsf2] = phi_prime_prime_2/a/H;
...
int background_output_budget(
  ...
  if (pba->background_verbose > 1) {
    ...
    if ((pba->has_lambda == _TRUE_) || (pba->has_fld == _TRUE_) || (pba->has_gsf == _TRUE_) || (pba->has_curvature == _TRUE_)) {
    ...
    if (pba->has_gsf == _TRUE_) {
      class_print_species("Scalar Field",gsf);
      budget_other+=pba->Omega0_gsf;
    }
    if (pba->has_gsf2 == _TRUE_) {
      class_print_species("Scalar Field 2",gsf2);
      budget_other+=pba->Omega0_gsf2;
    }
...
/**
 * Scalar field potential and its derivatives with respect to the field _gsf
 * For Albrecht & Skordis model: 9908085
 * - \f$ V = V_{p_{gsf}}*V_{e_{gsf}} \f$
...
// double V_e_gsf(struct background *pba,
//                double phi
//                ) {
//   double gsf_lambda = pba->gsf_parameters[0];
//   //  double gsf_alpha  = pba->gsf_parameters[1];
//   //  double gsf_A      = pba->gsf_parameters[2];
//   //  double gsf_B      = pba->gsf_parameters[3];

//   return  exp(-gsf_lambda*phi);
// }

// double dV_e_gsf(struct background *pba,
//                 double phi
//                 ) {
//   double gsf_lambda = pba->gsf_parameters[0];
//   //  double gsf_alpha  = pba->gsf_parameters[1];
...
//   return -gsf_lambda*V_gsf(pba,phi);
// }

// double ddV_e_gsf(struct background *pba,
//                  double phi
//                  ) {
//   double gsf_lambda = pba->gsf_parameters[0];
//   //  double gsf_alpha  = pba->gsf_parameters[1];
...
//   return pow(-gsf_lambda,2)*V_gsf(pba,phi);
// }
...
//  * double gsf_B = 34.8;
//  *
//  * double gsf_A = 0.01; (values for their Figure 2)
//  */
...
//   return  pow(phi - gsf_B,  gsf_alpha) +  gsf_A;
// }
...
//   return   gsf_alpha*pow(phi -  gsf_B,  gsf_alpha - 1);
// }
...
//   return  gsf_alpha*(gsf_alpha - 1.)*pow(phi -  gsf_B,  gsf_alpha - 2);
// }
...
//              struct background *pba,
//              double phi) {
//   return  V_e_gsf(pba,phi)*V_p_gsf(pba,phi);
// }

// double dV_gsf(
//               struct background *pba,
//               double phi) {
//   return dV_e_gsf(pba,phi)*V_p_gsf(pba,phi) + V_e_gsf(pba,phi)*dV_p_gsf(pba,phi);
// }

// double ddV_gsf(
//                struct background *pba,
//                double phi) {
//   return ddV_e_gsf(pba,phi)*V_p_gsf(pba,phi) + 2*dV_e_gsf(pba,phi)*dV_p_gsf(pba,phi) + V_e_gsf(pba,phi)*ddV_p_gsf(pba,phi);
// }
void gsf_functions(struct background *pba, double *gsf_in, double *gsf_out)
{
    double a = gsf_in[0];
    double H = gsf_in[1];
    double phi = gsf_in[2];
    double phi_prime = gsf_in[3];

    double P, PX, Pphi, PXX, PXphi, Pphiphi, PXXX, PXXphi, PXphiphi;
    double X = (phi_prime*phi_prime)/(2*a*a);

    //loop over all the models, and then calculate density and pressure
    double model_gsf = pba->gsf_parameters[0];

    if (model_gsf == 0)
    {//Quintessence (for late DE)

      double alpha = pba->gsf_parameters[5];
      double V0 = pba->gsf_parameters[6];
      // double X1 = 0.5* pow(phi_prime* pba->H0,2);

      P = X - V0* pow(phi, alpha);
      PX = 1;
      Pphi =  -V0* pow(phi, alpha-1)*alpha;
      PXX = 0;
      PXphi= 0;
      Pphiphi= -V0* pow(phi, alpha-2)*alpha*(alpha-1);
      PXXX= 0;
      PXXphi= 0; 
      PXphiphi= 0;
    } 
    ...
    //diatonic ghost field
    if (model_gsf == 1){
      double lambda = pba->gsf_parameters[5];
      double c1 = pba->gsf_parameters[6];
      // double X1 = 0.5* pow(phi_prime* pba->H0,2);

      P = - X + exp(lambda * phi) * X * X * c1 * pow(pba->H0, 0);
      PX = -1 + 2 * X * exp(lambda * phi) * c1* pow(pba->H0, 0);
      Pphi = lambda * exp(lambda * phi) * X * X * c1* pow(pba->H0, 0);
      PXX = 2 * exp(lambda * phi) * c1* pow(pba->H0, 0);
      PXphi= 2 * X * lambda * exp(lambda * phi) * c1* pow(pba->H0, 0);
      Pphiphi= lambda*lambda * exp(lambda * phi) * X * X * c1* pow(pba->H0, 0);
      PXXX=  0   ;
      PXXphi=  2 *lambda* exp(lambda * phi) * c1* pow(pba->H0, 0);  ;
      PXphiphi= 2 * X * lambda *lambda* exp(lambda * phi) * c1* pow(pba->H0, 0);  ;
    } 
    if (model_gsf == 2)
    {
      double alpha = pba->gsf_parameters[5];
      ...
      P = (- X + X * X )* pow(phi, -alpha)*c1;
      PX = (-1 + 2 * X )* pow(phi, -alpha)*c1 ;
      Pphi = (-alpha* pow(phi, -alpha-1) * (-X + X * X))*c1 ;
      PXX = 2 * c1 * pow(phi, -alpha);
      PXphi= -alpha* pow(phi, -alpha-1) * (-1 + 2 * X)*c1;
      Pphiphi= (-alpha* (-alpha-1)* pow(phi, -alpha-2) * (-X + X * X) )  *c1 ;
      PXXX=  0   ;
      PXXphi=  2 * c1 * pow(phi, -alpha-1) * (-alpha);
      PXphiphi= -alpha* pow(phi, -alpha-2) * (-alpha-1) * (-1 + 2 * X)*c1;
    } 
    if (model_gsf == 3)
    {
      double alpha = pba->gsf_parameters[5];
      double c1 = pba->gsf_parameters[6];
      double B = pba->gsf_parameters[7];
      // double X1 = 0.5* pow(phi_prime* pba->H0,2);

      P = (- X + X * X )* pow(phi, -alpha)*c1 +B*3.968e-8 ;
      PX = (-1 + 2 * X )* pow(phi, -alpha)*c1 ;
      Pphi = (-alpha* pow(phi, -alpha-1) * (-X + X * X))*c1 ;
      PXX = 2 * c1 * pow(phi, -alpha);
      PXphi= -alpha* pow(phi, -alpha-1) * (-1 + 2 * X)*c1;
    ...
    if (model_gsf == 4)
    {
      double n = pba->gsf_parameters[5];
      double c1 = pba->gsf_parameters[6];
      double lambda = pba->gsf_parameters[7];
      double V0 = pba->gsf_parameters[8]*pow(0.000225,2);
      // double X1 = 0.5* pow(phi_prime* pba->H0,2);

      P = - X + X * X * pow(phi, n)*c1 -  V0*pow(phi, lambda);//* pow(pba->H0/pba->h,2);
      PX = - 1 + 2 * X * pow(phi, n)*c1 ;//* pow(pba->H0/pba->h,2);
      Pphi = +X * X * pow(phi, n-1)*c1*n - lambda*V0*pow(phi, lambda-1);// * pow(pba->H0/pba->h,2);
      PXX = +2 * c1 * pow(phi, n);
      PXphi = +2 * X * pow(phi, n-1)*c1*n;
      Pphiphi = +X * X * pow(phi, n-2)*c1*n*(n-1) - lambda*lambda*V0*pow(phi, lambda-2);
      PXXX = 0;
      PXXphi = +2 * c1 * n *pow(phi, n-1);
      PXphiphi = +2 * X * pow(phi, n-2)*c1*n*(n-1);
    } 

    if (model_gsf == 5)
    { // Phantom Behavior: https://arxiv.org/pdf/astro-ph/9912463.pdf
      double alpha = pba->gsf_parameters[5];
      double V0 = pba->gsf_parameters[6];
      double A = pba->gsf_parameters[7];
      double B = pba->gsf_parameters[8];
      double C = pba->gsf_parameters[9];

      P =  V0 * pow(phi, -alpha) * (2*pow(X,4) - 4/3*A*pow(X,3) + B*pow(X,2) - C*X );
      PX = V0 * pow(phi, -alpha) * (8*pow(X,3) - 4*A*pow(X,2) + 2*B*pow(X,1) - C );
      Pphi = V0 * (-alpha)*pow(phi, -alpha-1) * (2*pow(X,4) - 4/3*A*pow(X,3) + B*pow(X,2) - C*X );
      PXX = V0 * pow(phi, -alpha) * (24*pow(X,2) - 8*A*pow(X,1) + 2*B );
      PXphi = V0 * (-alpha)*pow(phi, -alpha-1) * (8*pow(X,3) - 4*A*pow(X,2) + 2*B*pow(X,1) - C );
      Pphiphi = V0 * (-alpha)*(-alpha-1)*pow(phi, -alpha-2) * (2*pow(X,4) - 3/4*A*pow(X,3) + B*pow(X,2) - C*X );
      PXXX = V0 * pow(phi, -alpha) * (48*pow(X,1) - 8*A );
      PXXphi = V0 * (-alpha)*pow(phi, -alpha-1)* (24*pow(X,2) - 8*A*pow(X,1) + 2*B );
      PXphiphi = V0 *  (-alpha)*(-alpha-1)*pow(phi, -alpha-2) *(8*pow(X,3) - 4*A*pow(X,2) + 2*B*pow(X,1) - C );
    } 

    if (model_gsf == 6)
    {
      double A = pba->gsf_parameters[5];
      double V0 = pba->gsf_parameters[6];
      double m = pba->gsf_parameters[7];
      double cs2 =  pba->gsf_parameters[8]; //cs^2
      // double X1 = 0.5* pow(phi_prime* pba->H0,2);

      double n = (1-cs2)/(2*cs2); 

      P = pow(X, n+1) / pow(A,n) -V0*pow(phi, m);
      PX = (n+1) * pow(X, n) / pow(A,n) ;
      Pphi = -V0*pow(phi, m-1)*m;
      PXX = n* (n+1) * pow(X, n-1) / pow(A,n) ;
      PXphi= 0; 
      Pphiphi= -V0*pow(phi, m-2)*m*(m-1);
      PXXX= (n-1) * n* (n+1) * pow(X, n-2) / pow(A,n) ;
      PXXphi= 0; 
      PXphiphi= 0; 
    } 
    ...
    gsf_out[7] = PXXphi;
    gsf_out[8] = PXphiphi;
    gsf_out[9] = (-P + 2*X*PX)/3; //class-rho
    gsf_out[10] = P/3; //class pressure

   // double phidotdot = -(3*H*sqrt(2*X)*PX + PXphi*2*X - Pphi ) / ( PX+ 2*X*PXX   );
    double phidotdot = -(3*H*phi_prime*PX/a + PXphi*2*X - Pphi ) / ( PX+ 2*X*PXX   );
    gsf_out[11] = a*a*phidotdot + a*H*phi_prime; //phi''
  }


void gsf2_functions(struct background *pba, double *gsf2_in, double *gsf2_out)
{
    double a = gsf2_in[0];
    double H = gsf2_in[1];
    double phi = gsf2_in[2];
    double phi_prime = gsf2_in[3];

    double P, PX, Pphi, PXX, PXphi, Pphiphi, PXXX, PXXphi, PXphiphi;
    double X = (phi_prime*phi_prime)/(2*a*a);

    //loop over all the models, and then calculate density and pressure
    double model_gsf2 = pba->gsf2_parameters[0];

    if (model_gsf2 == 0)
    {//Rock and Roll Early Dark Energy
      double alpha = pba->gsf2_parameters[1];
      double V0 = pba->ede_p2;
    ...
    gsf2_out[7] = PXXphi;
    gsf2_out[8] = PXphiphi;
    gsf2_out[9] = (-P + 2*X*PX)/3; //class-rho
    gsf2_out[10] = P/3; //class pressure
    // double phidotdot = -(3*H*sqrt(2*X)*PX + PXphi*2*X - Pphi ) / ( PX+ 2*X*PXX   );
    double phidotdot = -(3*H*phi_prime*PX/a + PXphi*2*X - Pphi ) / ( PX+ 2*X*PXX   );
    gsf2_out[11] = a*a*phidotdot + a*H*phi_prime; //phi''
  }