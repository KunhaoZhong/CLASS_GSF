...
int input_shooting(struct file_content * pfc,
  ...
  char * const unknown_namestrings[] = {"h",                        /* unknown param for target '100*theta_s' */
                                        ...
                                        "gsf_shooting_parameter",   /* unknown param for target 'Omega_gsf' */
  ...
  enum computation_stage target_cs[] = {cs_thermodynamics, /* computation stage for target '100*theta_s' */
                                        ...
                                        cs_background,     /* computation stage for target 'omega_dcdmdr' */
                                        cs_background,     /* computation stage for target 'Omega_gsf' */
...
//  replace the input_find_root function above with the hi-class version
...
int input_get_guess(double *xguess,
  ...
  for (index_guess=0; index_guess < pfzw->target_size; index_guess++) {
    ...
    case Omega_gsf:
       ...
       * xguess[index_guess] = 1.77835*pow(ba.Omega0_gsf,-2./7.);
       * dxdy[index_guess] = -0.5081*pow(ba.Omega0_gsf,-9./7.)`;
       * Version 3: use attractor solution
       * */
//      if (ba.gsf_tuning_index == 0){
//        xguess[index_guess] = sqrt(3.0/ba.Omega0_gsf);
//        dxdy[index_guess] = -0.5*sqrt(3.0)*pow(ba.Omega0_gsf,-1.5);
//      }
//      else{
        /* Default: take the passed value as xguess and set dxdy to 1. */
        xguess[index_guess] = ba.gsf_parameters[ba.gsf_tuning_index];
        dxdy[index_guess] = ba.gsf_dxdy_guess;
        //dxdy[index_guess] = 1.;
//      }
      break;
    case omega_ini_dcdm:
...
int input_try_unknown_parameters(double * unknown_parameter,
  ...
  for (i=0; i < pfzw->target_size; i++) {
    ...
    case omega_dcdmdr:
      ...
      break;
    case Omega_gsf:
      /** In case scalar field is used to fill, pba->Omega0_gsf is not equal to pfzw->target_value[i].*/
      output[i] = ba.background_table[(ba.bt_size-1)*ba.bg_size+ba.index_bg_rho_gsf]/(ba.H0*ba.H0)-ba.Omega0_gsf;
      printf(" param[%i] = %e, Omega_gsf = %.3e, %.3e, target = %.2e, \n",ba.gsf_tuning_index, ba.gsf_parameters[ba.gsf_tuning_index],
	       ba.background_table[(ba.bt_size-1)*ba.bg_size+ba.index_bg_rho_gsf]
	          /ba.background_table[(ba.bt_size-1)*ba.bg_size+ba.index_bg_rho_crit], ba.background_table[(ba.bt_size-1)*ba.bg_size+ba.index_bg_rho_gsf]
...
int input_read_parameters_species(struct file_content * pfc,
  ...
  /* 7) ** ADDITIONAL SPECIES ** --> Add your species here */
    ...
    if (flag1 == _TRUE_){
      if(string_begins_with(string1,'y') || string_begins_with(string1,'Y')){
        pba->has_gsf2 = _TRUE_;    
        class_read_double("ede_p1",pba->ede_p1);
        class_read_double("ede_p2",pba->ede_p2);

        class_call(parser_read_string(pfc, "f_ede", &string1, &flag1, errmsg), errmsg,errmsg);
        class_call(parser_read_string(pfc, "zc_ede", &string1, &flag1, errmsg), errmsg,errmsg);
      }
      else{
        pba->has_gsf2  = _FALSE_;
 ...
 class_call(parser_read_list_of_doubles(pfc,
  ...
  class_call(parser_read_double(pfc,"Omega_fld",&param2,&flag2,errmsg),
             errmsg,
             errmsg);
  class_call(parser_read_double(pfc,"Omega_gsf",&param3,&flag3,errmsg),
             errmsg,
             errmsg);
  /* Test */
  class_test((flag1 == _TRUE_) && (flag2 == _TRUE_) && ((flag3 == _FALSE_) || (param3 >= 0.)),
             errmsg,
             "'Omega_Lambda' or 'Omega_fld' must be left unspecified, except if 'Omega_gsf' is set and < 0.");
  class_test(((flag1 == _FALSE_)||(flag2 == _FALSE_)) && ((flag3 == _TRUE_) && (param3 < 0.)),
             errmsg,
             "You have entered 'Omega_gsf' < 0 , so you have to specify both 'Omega_lambda' and 'Omega_fld'.");
  /* Complete set of parameters
     Case of (flag3 == _FALSE_) || (param3 >= 0.) means that either we have not
     read Omega_gsf so we are ignoring it (unlike lambda and fld!) OR we have
     ...
     We now proceed in two steps:
        ...
        2) go through the components in order {lambda, fld, gsf} and fill using
  ...
  if ((flag3 == _TRUE_) && (param3 >= 0.)){
    pba->Omega0_gsf = param3;
    Omega_tot += pba->Omega0_gsf;
  }
  /* Step 2 */
  if (flag1 == _FALSE_) {
    /* Fill with Lambda */
    ...
    pba->Omega0_gsf = 1. - pba->Omega0_k - Omega_tot;
    if (input_verbose > 0){
      printf(" -> matched budget equations by adjusting Omega_gsf = %g\n",pba->Omega0_gsf);
    }
  }
  ...
  /** 8.b) If Omega scalar field (gsf) is different from 0 */
  if (pba->Omega0_gsf != 0.){

    /** 8.b.1) Additional gsf parameters */
    /* Read */
    class_call(parser_read_list_of_doubles(pfc,
                                           "gsf_parameters",
                                           &(pba->gsf_parameters_size),
                                           &(pba->gsf_parameters),
                                           &flag1,
                                           errmsg),
               errmsg,errmsg);

     class_read_double("gsf_dxdy_guess",pba->gsf_dxdy_guess);
          printf("gsf_dxdy_guess = %f ",pba->gsf_dxdy_guess );
    /** 8.b.2) gsf initial conditions from attractor solution */
    /* Read */
    class_call(parser_read_string(pfc,
                                  "attractor_ic_gsf",
                                  &string1,
    ...
    if (flag1 == _TRUE_){
      if(string_begins_with(string1,'y') || string_begins_with(string1,'Y')){
        pba->attractor_ic_gsf = _TRUE_;
      }
      else{
        pba->attractor_ic_gsf = _FALSE_;
        /* Test */
        class_test(pba->gsf_parameters_size<2,
                   errmsg,
                   "Since you are not using attractor initial conditions, you must specify phi and its derivative phi' as the last two entries in gsf_parameters. See explanatory.ini for more details.");
        pba->phi_ini_gsf = pba->gsf_parameters[pba->gsf_parameters_size-2];
        pba->phi_prime_ini_gsf = pba->gsf_parameters[pba->gsf_parameters_size-1];
      }
    }

    /** 8.b.3) gsf tuning parameter */
    /* Read */
    class_read_int("gsf_tuning_index",pba->gsf_tuning_index);
    /* Test */
    class_test(pba->gsf_tuning_index >= pba->gsf_parameters_size,
               errmsg,
               "Tuning index 'gsf_tuning_index' (%d) is larger than the number of entries (%d) in 'gsf_parameters'.",
               pba->gsf_tuning_index,pba->gsf_parameters_size);

    /** 8.b.4) Shooting parameter */
    /* Read */
    class_read_double("gsf_shooting_parameter",pba->gsf_parameters[pba->gsf_tuning_index]);
    /* Complete set of parameters */
    gsf_lambda = pba->gsf_parameters[0];
    if ((fabs(gsf_lambda) < 3.)&&(pba->background_verbose>1)){
      printf("'gsf_lambda' = %e < 3 won't be tracking (for exp quint) unless overwritten by tuning function.",gsf_lambda);
    }
  }
...
int input_default_params(struct background *pba,
  ...
  pba->gsf_parameters = NULL;
  pba->gsf_parameters_size = 0;
  pba->gsf2_parameters = NULL;
  pba->gsf2_parameters_size = 0;
  /** 9.b.2) Initial conditions from attractor solution */
  pba->attractor_ic_gsf = _FALSE_;
  pba->phi_ini_gsf = 1;                // MZ: initial conditions are as multiplicative
  pba->phi_prime_ini_gsf = 1;          //     factors of the radiation attractor values
  /** 9.b.3) Tuning parameter */
  pba->gsf2_tuning_index = 0;
  pba->attractor_ic_gsf2 = _FALSE_;
  /** 9.b.3) Tuning parameter */
  pba->gsf_tuning_index = 0;
